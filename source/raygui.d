...
enum RAYGUI_VERSION_MINOR = 6;
enum RAYGUI_VERSION =  "3.6";

import raylib;
public import raylib: Rectangle, Font, Image;
...
private bool CHECK_BOUNDS_ID(Rectangle src, Rectangle dst) {
    return (src.x == dst.x) && (src.y == dst.y) && (src.width == dst.width) && (src.height == dst.height);
}
...
private Rectangle guiSliderActive;       // Gui slider active bounds rectangle, used as an unique identifier

private uint textBoxCursorIndex = 0;             // Cursor index, shared by all GuiTextBox*()
private int autoCursorCooldownCounter = 0;      // Cooldown frame counter for automatic cursor movement on key-down
private int autoCursorDelayCounter = 0;         // Delay frame coutner for automatic cursor movement
...
enum RAYGUI_TABBAR_ITEM_WIDTH =    160;
    ...
    int offsetX = (*active + 2)*RAYGUI_TABBAR_ITEM_WIDTH - GetScreenWidth();
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth()) {
            ...
            // NOTE: Only draw close button for current tab: if (CheckCollisionPointRec(mousePoint, tabBounds))
            int tempBorderWidth = GuiGetStyle(BUTTON, BORDER_WIDTH);
            int tempTextAlignment = GuiGetStyle(BUTTON, TEXT_ALIGNMENT);
            GuiSetStyle(BUTTON, BORDER_WIDTH, 1);
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
version (RAYGUI_NO_ICONS) {
            if (GuiButton(Rectangle(tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14), "x")) closing = i;
} else {
...
version (SUPPORT_SCROLLBAR_KEY_INPUT) {
            ...
            if (hasHorizontalScrollBar && (IsKeyDown(KeyboardKey.KEY_LEFT_CONTROL) || IsKeyDown(KeyboardKey.KEY_LEFT_SHIFT))) scrollPos.x += wheelMove*20;
...
int GuiComboBox(Rectangle bounds, const(char)* text, int active, bool editMode=false) {
    GuiState state = guiState;
...
bool GuiTextBox(Rectangle bounds, char* text, int bufferSize, bool editMode) {
    const uint AUTO_CURSOR_COOLDOWN = 40;
    const uint AUTO_CURSOR_DELAY = 1;
    ...
    final switch (alignmentVertical)
    {
        ...
        case 3: cursor.y = textBounds.y + textBounds.height; break;
    }
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (editMode)
        {
            ...
            uint textLength = cast(uint)text.strlen;   // Get current text length
            ...
            if ((textLength > 0) && IsKeyPressed(KeyboardKey.KEY_HOME))
            {
                textBoxCursorIndex = 0;
            }

            // Move cursor to end
            if ((textLength > textBoxCursorIndex) && IsKeyPressed(KeyboardKey.KEY_END))
            {
                textBoxCursorIndex = textLength;
            }

            // Delete codepoint from text, after current cursor position
            if ((textLength > textBoxCursorIndex) && (IsKeyPressed(KeyboardKey.KEY_DELETE) || (IsKeyDown(KeyboardKey.KEY_DELETE) && (autoCursorCooldownCounter >= AUTO_CURSOR_COOLDOWN))))
            {
                autoCursorDelayCounter++;

                if (IsKeyPressed(KeyboardKey.KEY_DELETE) || (autoCursorDelayCounter%AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int nextCodepointSize = 0;
           ...
           // Delete codepoint from text, before current cursor position
            if ((textLength > 0) && (IsKeyPressed(KeyboardKey.KEY_BACKSPACE) || (IsKeyDown(KeyboardKey.KEY_BACKSPACE) && (autoCursorCooldownCounter >= AUTO_CURSOR_COOLDOWN))))
            {
                autoCursorDelayCounter++;

                if (IsKeyPressed(KeyboardKey.KEY_BACKSPACE) || (autoCursorDelayCounter%AUTO_CURSOR_DELAY) == 0)     // Delay every movement some frames 
                {
            ...
            if (IsKeyPressed(KeyboardKey.KEY_LEFT) || (IsKeyDown(KeyboardKey.KEY_LEFT) && (autoCursorCooldownCounter > AUTO_CURSOR_COOLDOWN)))
            {
                autoCursorDelayCounter++;

                if (IsKeyPressed(KeyboardKey.KEY_LEFT) || (autoCursorDelayCounter%AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames 
                {
                    int prevCodepointSize = 0;
                    GetCodepointPrevious(text + textBoxCursorIndex, &prevCodepointSize);
            ...
            else if (IsKeyPressed(KeyboardKey.KEY_RIGHT) || (IsKeyDown(KeyboardKey.KEY_RIGHT) && (autoCursorCooldownCounter > AUTO_CURSOR_COOLDOWN)))
            {
                autoCursorDelayCounter++;
                if (IsKeyPressed(KeyboardKey.KEY_RIGHT) || (autoCursorDelayCounter%AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames 
                {
                    int nextCodepointSize = 0;
                    GetCodepointNext(text + textBoxCursorIndex, &nextCodepointSize);
    ...
    if (editMode)
    {
        ...
        GuiDrawRectangle(cursor, 0, Colors.BLANK, Fade(GetColor(GuiGetStyle(TEXTBOX, BORDER_COLOR_PRESSED)), guiAlpha));
    }
...
//bool GuiTextBoxMulti // D Note: This is commented out in raygui, but it should be added to Raylib-D if it's uncommented in raygui.
...
bool GuiValueBox(Rectangle bounds, const(char)* text, int* value, int minValue, int maxValue, bool editMode) {
    ...
    if (text != null)
    {
        textBounds.width = cast(float)GetTextWidth(text) + 2.0f;
        textBounds.height = cast(float)GuiGetStyle(DEFAULT, TEXT_SIZE);
...
Vector2 GuiGrid(Rectangle bounds, const(char)* text, float spacing, int subdivs) {
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            currentCell.x = floor((mousePoint.x - bounds.x)/spacing);
            currentCell.y = floor((mousePoint.y - bounds.y)/spacing);
        }
    }
...
// Set tooltip string
void GuiSetTooltip(const(char)* tooltip) { guiTooltipPtr = cast(char*) tooltip; }
...
void GuiLoadStyle(const(char)* fileName) {
    ...
    // D Note: This should be rewritten for a safer format.
    FILE* rgsFilePtr = fopen(fileName, "rt");
    //File rgsFile = File(fileName.fromStringz, "r");

    if (rgsFilePtr != null)
    {
        char[MAX_LINE_BUFFER_SIZE] buffer = 0;
        fgets(buffer.ptr, MAX_LINE_BUFFER_SIZE, rgsFilePtr);
    ...
    if (tryBinary)
    {
        rgsFilePtr = fopen(fileName, "rb");

        if (rgsFilePtr == null)
        {
            fseek(rgsFilePtr, 0, SEEK_END);
            uint fileDataSize = cast(uint)ftell(rgsFilePtr);
            fseek(rgsFilePtr, 0, SEEK_SET);

            if (fileDataSize > 0)
            {
                //D Note: The following line is for a later change to be more D-like
                //auto fileData = rgsFile.byChunk;
                char* fileData = cast(char*)malloc(fileDataSize*char.sizeof);
                fread(fileData, char.sizeof, fileDataSize, rgsFilePtr);

                // D Note: Check on this one as it's converting `char*` to `ubyte*`. There may be a better solution.
                //auto data = (cast(char[])fileData).byChunk;
...
void GuiLoadStyleDefault() {
    ...
    GuiSetStyle(TEXTBOX, TEXT_LINES_SPACING, cast(int)(cast(float)GuiGetStyle(DEFAULT, TEXT_SIZE)*1.5f));
...
// D note: This may be reworked to make it safer.
private void GuiLoadStyleFromMemory(const(char)* fileData, int dataSize)
{
    char* fileDataPtr = cast(char*)fileData;
        ...
        (signature[3] == ' '))
    {
        ...
        for (int p = 0; p < propertyCount; p++)
        {
            ...
            if (controlId == 0) // DEFAULT control
            {
                ...
                if (propertyId < RAYGUI_MAX_PROPS_BASE) for (int i = 1; i < RAYGUI_MAX_CONTROLS; i++) GuiSetStyle(i, cast(int)propertyId, propertyValue);
            }
...
static if (!HasVersion!"RAYGUI_STANDALONE") {
        ...
        if (fontDataSize > 0)
        {
            ...
            memcpy(&whiteRec, fileDataPtr, Rectangle.sizeof);
            fileDataPtr += 16;
            ...
            if (fontImageCompSize < fontImageUncompSize)
            {
                ...
                char* compData = cast(char*)malloc(fontImageCompSize);
            ...
            else
            {
                ...
                imFont.data = cast(ubyte*)malloc(fontImageUncompSize);
            ...
            // Load font recs data
            font.recs = cast(Rectangle*)calloc(font.glyphCount, Rectangle.sizeof);
            ...
            font.glyphs = cast(GlyphInfo*)calloc(font.glyphCount, GlyphInfo.sizeof);
            for (int i = 0; i < font.glyphCount; i++)
            {
